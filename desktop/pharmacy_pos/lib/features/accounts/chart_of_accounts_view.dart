    ...
    content: SizedBox(width: 560, child: Form(key: _form, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      if (!editing) ...[
        const SizedBox(height: 12), Row(children: [
          Expanded(child: DropdownButtonFormField<String>(initialValue: _type, decoration: const InputDecoration(labelText: 'Account type'), items: const ['Asset','Liability','Equity','Income','CostOfSales','Expense'].map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(), onChanged: (x) => setState(() => _type = x!))),
          const SizedBox(width: 12), Expanded(child: DropdownButtonFormField<String>(initialValue: _normal, decoration: const InputDecoration(labelText: 'Normal balance'), items: const ['Debit','Credit'].map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(), onChanged: (x) => setState(() => _normal = x!))),
        ]),
        const SizedBox(height: 12), DropdownButtonFormField<String?>(initialValue: _parent, decoration: const InputDecoration(labelText: 'Parent account (optional)'), items: [const DropdownMenuItem<String?>(value: null, child: Text('No parent')), ...widget.accounts.where((x) => !x.isPostingAccount).map((x) => DropdownMenuItem(value: x.id, child: Text('${x.code} · ${x.name}')))], onChanged: (x) => setState(() => _parent = x)),
      ],
