...
There are 28 mapped application entities/tables. PostgreSQL also creates `__EFMigrationsHistory` when migrations are applied.
...
- Purchase order numbers, GRN numbers, supplier invoice numbers, sale invoice numbers, held sale numbers, sales-return numbers, and purchase-return numbers have unique indexes scoped to their business rules.
- FEFO, branch, active-state, audit, stock-ledger, supplier, supplier-ledger, purchase-order, goods-receipt, purchase-return, POS search, sales history, sale payment, sales-return, refund-payment, and receipt query paths have supporting indexes.
...
- Supplier opening balance, credit limit, supplier ledger amounts, purchase prices, purchase receipt totals, purchase-return totals, sale totals, sales-return totals, and refund payments use `decimal(18,2)`.
...
- No exchange/store-credit return flow, receipt-less return flow, customer credit billing, supplier cash-refund settlement for purchase returns, transfers, reports, unit conversion, accounting general ledger, supplier payment allocation, shift/cash drawer closing, or background expiry processing.
